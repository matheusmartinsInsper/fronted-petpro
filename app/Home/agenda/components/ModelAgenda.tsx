...
export const ModalAgenda: React.FC<{ isOpen: boolean, onClose: () => void, toggleStateapi: string }> = ({ isOpen, onClose, toggleStateapi }) => {
  ...
  const handleUserInfoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPayload((prev) => ({
      ...
      user: {
        ...prev.user,
        [name]: value, // Atualiza os dados gerais como nome, email e telefone
      },
    }));
  };
  const handleAddressChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPayload((prev) => ({
      ...
      user: {
        ...
        adress: {
          ...prev.user.adress,
          [name]: value, // Atualiza os campos de endereço como uf, city, street, etc.
        },
      },
    }));
  };
  ...
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    // Remove caracteres não numéricos e mantém o valor como string
    const rawValue = e.target.value.replace(/\D/g, "");

    // Atualiza o estado, convertendo os valores necessários para números
    setPayload((prev) => ({
      ...prev,
      user: {
        ...prev.user,
        phone: {
          phoneRaw: rawValue, // Armazena o número de telefone como string (rawValue)
          areacode: parseInt(rawValue.slice(0, 2)) || 0, // Código de área como número
          phone: parseInt(rawValue.slice(2)) || 0, // Número do telefone como número
          countrycode: 55, // Código do país fixo
        },
      },
    }));
  };
  ...
  const handlepaginat = (stage: number) => {
    setStep(stage);
  }
  const handleBack = () => {
    setStep(1);
      ...
      <ModalContent minWidth={"620px"}>
        ...
        <ModalBody>
          {step === 1 && (
            <Box border="1px" borderColor="gray.200" borderRadius="md" p={4} width={"100%"} boxShadow={"md"}>
              ...
              <Flex wrap="wrap" justify="space-between" mb={4} flexDirection={"column"}>
                <Box display={"flex"} flexDirection={"row"} mb={4}>
                  <Box flex="1" pr={4}>
                    ...
                    <Input
                      ...
                      borderRadius={"md"}
                      onChange={handleUserInfoChange} // Usando o método para nome, email, telefone
                  ...
                  <Box flex="1" pr={4}>
                    <Text fontSize={"sm"} fontWeight={"bold"}><strong>Telefone:</strong></Text>
                    <Input
                      name="phone"
                      type="text"  // Mudando para 'text' para permitir entradas de caracteres
                      size="sm"
                      value={payload.user.phone.phoneRaw || ""}  // Usando o valor bruto armazenado
                ...
                <Box flex="1" display={"flex"} flexDirection={"row"}>
                  <FormControl mb={4} pr={4}>
                    <FormLabel fontSize={"sm"} fontWeight={"bold"}>CEP</FormLabel>
                    <Input
                      size="sm"
                      name="cep"
                      value={payload.user.adress.cep}
                  ...
                  <FormControl mb={4} pr={4}>
                    <FormLabel fontSize={"sm"} fontWeight={"bold"}>UF</FormLabel>
                    <Input
                      size="sm"
                      name="uf"
                      value={payload.user.adress.uf}
                      onChange={handleAddressChange}
                      placeholder="Ex: SP"
                      focusBorderColor="primary.400"
                      borderRadius={"md"}
                  ...
                  <FormControl mb={4} pr={4}>
                    <FormLabel fontSize={"sm"} fontWeight={"bold"}>Cidade</FormLabel>
                    <Input
                      size="sm"
                      name="city"
                      value={payload.user.adress.city}
                      onChange={handleAddressChange}
                ...
                <Box flex="1" display={"flex"} flexDirection={"row"}>
                  <FormControl mb={4} pr={4}>
                    <FormLabel fontSize={"sm"} fontWeight={"bold"}>Rua</FormLabel>
                    <Input
                      size="sm"
                      name="street"
                      value={payload.user.adress.street}
                      onChange={handleAddressChange}
                  ...
                  <FormControl mb={4} pr={4}>
                    <FormLabel fontSize={"sm"} fontWeight={"bold"}>Número</FormLabel>
                    <Input
                      size="sm"
                      name="number"
                      type="number"
                      value={payload.user.adress.number}
                      onChange={handleAddressChange}
                      placeholder="Ex: 123"
                      focusBorderColor="primary.400"
                      borderRadius={"md"}
              ...
              <Flex justify="flex-end" mt={4} pr={4}>
                <Button _hover={{ backgroundColor: "primary.300", color: "primary.100" }} colorScheme="blue" onClick={() => handlepaginat(2)} bgColor={"white"} size={"sm"} color={"primary.200"} boxShadow={"md"}>
                  Próximo
                </Button>
              </Flex>
          ...
          {step === 2 && (
            <Box border="1px" borderColor="gray.200" borderRadius="md" p={4} width={"100%"} boxShadow={"md"}>
              ...
              <Flex justifyContent={"space-between"}>
                <Button bgColor={"white"} size={"sm"} color={"primary.200"} boxShadow={"md"} _hover={{ backgroundColor: "primary.300", color: "primary.100" }} onClick={() => handlepaginat(1)} >
                  Voltar
                </Button>
                <Button bgColor={"white"} size={"sm"} color={"primary.200"} boxShadow={"md"} _hover={{ backgroundColor: "primary.300", color: "primary.100" }} onClick={() => handlepaginat(3)}>
                  Próximo
                </Button>
              </Flex>
        ...
        <ModalFooter>
          <Flex width="100%" justify="space-between">
            ...
            {step === 3 && (
              <>
                <Button bgColor={"white"} size={"sm"} color={"primary.200"} boxShadow={"md"} _hover={{ backgroundColor: "primary.300", color: "primary.100" }} onClick={() => handlepaginat(2)} >
                  Voltar
                </Button>
                <Link>
                  <Button
